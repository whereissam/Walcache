...
function getMockBlobStatus(chain: SupportedChain, blobId?: string): ChainBlobStatus {
...
export function ChainSelector({ 
  ...
  const [blobStatuses, setBlobStatuses] = React.useState<Record<SupportedChain, ChainBlobStatus>>({});
      ...
      <CardContent>
              ...
              >
                ...
                {showBlobStatus && blobStatus?.lastChecked && (
                  <p className="text-xs text-gray-500 w-full text-left">
                    ...
                    Checked {Math.round((new Date().getTime() - blobStatus.lastChecked.getTime()) / 1000)}s ago
                  </p>
                )}
              </Button>
            );
        ...
        {showBlobStatus && Object.keys(blobStatuses).length > 0 && (
          <div className="mt-4 p-3 bg-gray-50 rounded-lg">
            <h4 className="text-sm font-medium mb-2">Multi-Chain Availability Summary</h4>
            ...
            <div className="mt-2 text-xs text-blue-600">
              💡 <strong>Pro Tip:</strong> Use SDK function <code>getWalrusCDNUrl(blobId, &#123;chain&#125;)</code> to automatically get the best URL
            </div>
          </div>
        )}
      </CardContent>

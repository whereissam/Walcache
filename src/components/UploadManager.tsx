...
export function UploadManager() {
  ...
  const [directUploads, setDirectUploads] = useState<Array<{
    blobId: string;
    fileName: string;
    size: number;
    contentType: string;
  ...
  const handleFileUpload = useCallback(async (fileList: FileList) => {
    ...
    try {
      ...
      if (result.verified) {
        ...
        console.log(`⚠️ File uploaded to Vault but blob not yet available on Walrus`);
        // Mark as syncing
        setFileStatuses(prev => ({
          ...prev,
          [result.file.blobId]: {
  ...
  const handleWalrusUpload = useCallback(async (fileList: FileList) => {
    if (fileList.length === 0) return;
    ...
    try {
      ...
      if (result.verified) {
        console.log(`✅ Direct Walrus upload successful and verified on ${result.network}!`);
        ...
        console.log(`⚠️ Direct Walrus upload completed but blob not yet available on aggregators`);
        console.log('Upload result:', result.upload);
        ...
        alert(`⏳ Upload successful but still syncing\n\nBlob ID: ${result.upload.blobId}\nStatus: ${result.upload.status}\n\nThe blob may take a few minutes to appear on aggregators.`);
      }
    } catch (error) {
      console.error('Direct Walrus upload failed:', error);
      setError(error instanceof Error ? error.message : 'Direct Walrus upload failed');
    }
  ...
  const copyToClipboard = async (url: string, type: 'cdn' | 'blobId' | 'aggregator' | 'download') => {
    try {
  ...
  const checkWalrusStatus = async (blobId: string, retryCount = 0): Promise<boolean | null> => {
    const existing = fileStatuses[blobId];
    const now = new Date();
    
    // Return cached result if checked within last 5 minutes
    if (existing?.lastChecked && (now.getTime() - existing.lastChecked.getTime()) < 5 * 60 * 1000) {
    ...
    try {
      ...
      for (const aggregator of WALRUS_AGGREGATORS) {
        try {
          ...
          if (response.ok) {
            ...
            const network = aggregator.includes('testnet') ? 'testnet' : 'mainnet';
            setFileStatuses(prev => ({ 
              ...prev, 
              [blobId]: { 
                ...prev[blobId], 
      ...
      if (retryCount < 2 && (error instanceof TypeError || error.name === 'NetworkError')) {
        console.log(`Retrying Walrus check for ${blobId}, attempt ${retryCount + 1}`);
  ...
  const checkAllWalrusStatuses = async (fileList: typeof files) => {
    ...
    const uncheckedFiles = fileList.filter(file => {
      ...
      return !status?.lastChecked || (now.getTime() - status.lastChecked.getTime()) > 5 * 60 * 1000;
    });
                ...
                <SelectContent>
                  {vaults.length === 0 && !isLoading ? (
                    ...
                    vaults.map((vault) => {
                      ...
                      const currentFileCount = files.filter(f => f.vaultId === vault.id).length;
                      return (
                        <SelectItem key={vault.id} value={vault.id}>
                          <div className="flex items-center space-x-2">
                            <Database className="h-4 w-4 text-gray-400" />
                            <span>{truncateText(vault.name, 30)} ({currentFileCount} files)</span>
                          </div>
                        </SelectItem>
                      );
                    })
        ...
        <CardContent>
          ...
          >
            ...
            <input
              ...
              id="walrus-upload"
              className="hidden"
              onChange={(e) => e.target.files && handleWalrusUpload(e.target.files)}
            />
            ...
            <div key={id} className="mt-4 p-3 border rounded-lg">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-gray-500">
                  {upload.status === 'uploading' && upload.progress < 80 && `Uploading ${upload.progress}%`}
                  {upload.status === 'uploading' && upload.progress >= 80 && 'Verifying on Walrus...'}
                            ...
                            <div className="flex items-center space-x-1">
                              ...
                              {lastChecked && (
                                <span className="text-xs text-gray-400">
                                  {Math.round((new Date().getTime() - lastChecked.getTime()) / 60000)}分前檢查
                                </span>
                              )}
                            </div>
                          )}
                          ...
                          {walrusExists === undefined && (
                            <button 
                              onClick={async () => {
                                ...
                                const result = await checkBlobOnWalrus(file.blobId);
                                setFileStatuses(prev => ({
                                  ...prev,
                                  [file.blobId]: {
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const fileStatus = fileStatuses[file.blobId];
                          const aggregator = fileStatus?.workingAggregator || WALRUS_AGGREGATORS[0];
                        ...
                        className="w-full sm:w-auto text-xs"
                      >
                        {copiedUrl?.startsWith('aggregator:') && copiedUrl.includes(file.blobId) ? (
                          <Check className="h-3 w-3 text-green-600 mr-1" />
                        ) : (
                          <Copy className="h-3 w-3 mr-1" />
                        )}
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          setShowingLinksFor(showingLinksFor === file.id ? null : file.id);
                        }}
                      ...
                      <div className="space-y-2">
                        ...
                        {file.cdnUrl && (
                          <div className="flex items-center justify-between p-2 bg-white rounded border">
                            <div>
                              <span className="text-xs font-medium text-green-700">CDN URL (Cached)</span>
                              <div className="text-xs text-gray-600 font-mono break-all">{file.cdnUrl}</div>
                            </div>
                            <div className="flex space-x-1 ml-2">
                              <Button
                                ...
                                className="text-xs"
                              >
                                {copiedUrl === 'cdn:' + file.cdnUrl ? '✓' : 'Copy'}
                              </Button>
                              <Button
                                size="sm"
                                variant="outline"
                        ...
                        {(file as any).downloadUrl && (
                          <div className="flex items-center justify-between p-2 bg-white rounded border">
                            <div>
                              <span className="text-xs font-medium text-blue-700">Download URL (Backend)</span>
                              <div className="text-xs text-gray-600 font-mono break-all">{(file as any).downloadUrl}</div>
                            </div>
                            <div className="flex space-x-1 ml-2">
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => copyToClipboard((file as any).downloadUrl, 'download')}
                                className="text-xs"
                              >
                                {copiedUrl === 'download:' + (file as any).downloadUrl ? '✓' : 'Copy'}
                              </Button>
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => window.open((file as any).downloadUrl, '_blank')}
                                className="text-xs"
                              >
                                Open
                              </Button>
                        ...
                        <div className="flex items-center justify-between p-2 bg-white rounded border">
                          <div>
                            <span className="text-xs font-medium text-purple-700">
                              Direct Walrus URL ({fileStatuses[file.blobId]?.foundOnNetwork || 'testnet'})
                            </span>
                            <div className="text-xs text-gray-600 font-mono break-all">
                              {fileStatuses[file.blobId]?.workingAggregator || WALRUS_AGGREGATORS[0]}/v1/blobs/{file.blobId}
                            </div>
                            {fileStatuses[file.blobId]?.walrusExists === false && (
                              <div className="text-xs text-orange-600 mt-1">⚠️ May return 404 - still syncing</div>
                            )}
                          </div>
                          <div className="flex space-x-1 ml-2">
                            <Button
                              ...
                              variant="outline"
                              onClick={() => {
                                const aggregator = fileStatuses[file.blobId]?.workingAggregator || WALRUS_AGGREGATORS[0];
                                const url = `${aggregator}/v1/blobs/${file.blobId}`;
                                copyToClipboard(url, 'aggregator');
                              }}
                              className="text-xs"
                            >
                              {copiedUrl?.startsWith('aggregator:') && copiedUrl.includes(file.blobId) ? '✓' : 'Copy'}
                            </Button>
                            <Button
                              size="sm"
                              variant="outline"
                              ...
                              className="text-xs"
                              disabled={fileStatuses[file.blobId]?.walrusExists === false}
                            >
                              Open
                            </Button>
                          </div>
                        ...
                        <div className="flex items-center justify-between p-2 bg-white rounded border">
                          <div>
                            <span className="text-xs font-medium text-gray-700">Blob ID</span>
                            <div className="text-xs text-gray-600 font-mono break-all">{file.blobId}</div>
                          </div>
                          <div className="flex space-x-1 ml-2">
                            <Button
                              size="sm"
                              ...
                              className="text-xs"
                            >
                              {copiedUrl === 'blobId:' + file.blobId ? '✓' : 'Copy'}
                            </Button>
                          </div>
                        </div>
                      </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {directUploads.map((upload) => {
                ...
                const walrusExists = fileStatus?.walrusExists;
                return (
                  <div key={upload.blobId} className="border rounded-lg p-4 bg-blue-50">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2">
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => copyToClipboard(upload.blobId, 'blobId')}
                          title="Copy Blob ID (Walrus identifier)"
                          className="w-full sm:w-auto text-xs"
                        >
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => copyToClipboard(upload.directUrl, 'aggregator')}
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            setShowingLinksFor(showingLinksFor === upload.blobId ? null : upload.blobId);
                          }}
                          title="Show all available links"
                          className="w-full sm:w-auto text-xs"
                        >
                          <ExternalLink className="h-3 w-3 mr-1" />
                          {showingLinksFor === upload.blobId ? 'Hide' : 'Show'} Links
                        </Button>
                      </div>
                    </div>
                      ...
                      <div className="mt-4 p-3 bg-white rounded-lg border-t">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center justify-between p-2 bg-gray-50 rounded border">
                            <div>
                              <span className="text-xs font-medium text-green-700">CDN URL (Cached)</span>
                              <div className="text-xs text-gray-600 font-mono break-all">{upload.cdnUrl}</div>
                            </div>
                            <div className="flex space-x-1 ml-2">
                              <Button
                                size="sm"
                                ...
                                className="text-xs"
                              >
                                {copiedUrl === 'cdn:' + upload.cdnUrl ? '✓' : 'Copy'}
                              </Button>
                              <Button
                                size="sm"
                                variant="outline"
                          ...
                          <div className="flex items-center justify-between p-2 bg-gray-50 rounded border">
                            <div>
                              <span className="text-xs font-medium text-purple-700">Direct Walrus URL</span>
                              <div className="text-xs text-gray-600 font-mono break-all">{upload.directUrl}</div>
                              {walrusExists === false && (
                                <div className="text-xs text-orange-600 mt-1">⚠️ May return 404 - still syncing</div>
                              )}
                            </div>
                            <div className="flex space-x-1 ml-2">
                              <Button
                                ...
                                className="text-xs"
                              >
                                {copiedUrl === 'aggregator:' + upload.directUrl ? '✓' : 'Copy'}
                              </Button>
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => window.open(upload.directUrl, '_blank')}
                                className="text-xs"
                          ...
                          <div className="flex items-center justify-between p-2 bg-gray-50 rounded border">
                            <div>
                              <span className="text-xs font-medium text-gray-700">Blob ID</span>
                              <div className="text-xs text-gray-600 font-mono break-all">{upload.blobId}</div>
                            </div>
                            <div className="flex space-x-1 ml-2">
                              <Button
                                size="sm"
                                ...
                                className="text-xs"
                              >
                                {copiedUrl === 'blobId:' + upload.blobId ? '✓' : 'Copy'}
                              </Button>
                            </div>
                          </div>
                        </div>

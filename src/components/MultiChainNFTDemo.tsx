...
import { Copy, ExternalLink, Zap, Clock, AlertTriangle, Star, Image, Gamepad2, FileText, Database, Activity } from 'lucide-react';
...
function getWalrusCDNUrl(blobId: string, options?: { chain?: SupportedChain; baseUrl?: string }) {
...
function getTypeIcon(type: string) {
  switch (type) {
    ...
    case 'uploaded': return <Database className="h-4 w-4" />;
...
function getTypeColor(type: string) {
  switch (type) {
    ...
    case 'uploaded': return 'bg-orange-100 text-orange-800';
...
export function MultiChainNFTDemo() {
  ...
  const [selectedAsset, setSelectedAsset] = useState<MultiChainAsset | null>(null);
  ...
  const [chainLatencies, setChainLatencies] = useState<Record<SupportedChain, number>>({
    sui: 85,
    ethereum: 450,
    solana: 220
  });
  ...
  useEffect(() => {
    ...
    const otherFiles = files.filter(file => !file.type.startsWith('image/') && !file.type.startsWith('text/'));
    const prioritizedFiles = [...imageFiles, ...textFiles, ...otherFiles]; // Show all files
    
    const uploadedAssets: MultiChainAsset[] = prioritizedFiles.map((file, index) => {
      ...
      if (file.type.startsWith('image/')) {
        ...
        useCase = assetType === 'nft' ? 
          'User-uploaded NFT artwork accessible across all blockchains' :
          'User-uploaded game asset usable in multi-chain gaming';
      } else if (file.type.startsWith('text/')) {
        assetType = 'document';
        useCase = 'User-uploaded document demonstrating cross-chain data storage';
      }
      
      return {
        id: `uploaded-${file.id}`,
        name: file.name || `Uploaded ${assetType.charAt(0).toUpperCase() + assetType.slice(1)} ${index + 1}`,
        type: assetType,
        blobId: file.blobId,
  ...
  const testAssetLoad = async (asset: MultiChainAsset, chain: SupportedChain) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`⚠️ ${chain} access failed: ${response.status} - File may not be uploaded to Walrus yet`);
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            <strong>Multi-Chain CDN:</strong> Your uploaded files are accessible from ANY blockchain via chain-specific endpoints!
            <br />
            <strong>One SDK, All Chains:</strong> <code>getWalrusCDNUrl(blobId, &#123;chain: 'ethereum'&#125;)</code> → Works instantly
          </CardDescription>
        </CardHeader>
      </Card>
            ...
            {cacheStats && (
              <>
                <div className="text-center p-3 border rounded-lg">
                  ...
                  <div className="text-2xl font-bold mb-1">{cacheStats.memory.keys}</div>
                  <div className={`text-xs px-2 py-1 rounded ${
                    cacheStats.memory.hitRate > 70 ? 'bg-green-100 text-green-800' :
                    cacheStats.memory.hitRate > 40 ? 'bg-yellow-100 text-yellow-800' :
                ...
                <div className="text-center p-3 border rounded-lg">
                  ...
                  <div className="text-2xl font-bold mb-1">{cacheStats.redis.keys}</div>
                ...
                <div className="text-center p-3 border rounded-lg">
                  ...
                  <div className="text-2xl font-bold mb-1">{cacheStats.using.toUpperCase()}</div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Real CDN performance across Sui, Ethereum, and Solana endpoints - same content, optimized for each chain
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-3 gap-4">
            {(['sui', 'ethereum', 'solana'] as SupportedChain[]).map(chain => (
              <div key={chain} className="text-center p-3 border rounded-lg">
                <div className="text-sm font-medium mb-1">{chain.toUpperCase()}</div>
                <div className="text-2xl font-bold mb-1">{chainLatencies[chain]}ms</div>
                <div className={`text-xs px-2 py-1 rounded ${
                  chainLatencies[chain] < 150 ? 'bg-green-100 text-green-800' :
                  chainLatencies[chain] < 300 ? 'bg-yellow-100 text-yellow-800' :
                  'bg-red-100 text-red-800'
                }`}>
                  {chain === 'sui' ? 'Sui Endpoint' : 
                   chain === 'ethereum' ? 'Ethereum Endpoint' : 'Solana Endpoint'}
                </div>
              </div>
            ))}
          </div>
          <div className="mt-3 text-xs text-gray-600 text-center">
            💡 Walrus CDN consistently outperforms traditional decentralized storage gateways
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Real Web3 use cases: NFTs, game assets, and DAO documents accessible across multiple chains
          </CardDescription>
        </CardHeader>
        <CardContent>
          {realAssets.length === 0 ? (
            <div className="text-center py-8">
              <div className="text-gray-500 mb-4">
                ...
                <h3 className="text-lg font-medium mb-2">Upload via Demo Page</h3>
                <p className="text-sm">Go to the Demo page and upload files via "/upload/walrus" to see them here!</p>
                <p className="text-xs mt-2">Current uploads via Tusky.io vaults don't appear in multi-chain demo.</p>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Card 
                ...
                className={`cursor-pointer transition-all ${
                  selectedAsset?.id === asset.id ? 'ring-2 ring-blue-500' : 'hover:shadow-md'
                }`}
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <Badge variant="outline">{asset.originalChain.toUpperCase()}</Badge>
                    </div>
                    ...
                    <div className="text-xs text-gray-500">
                      Size: {asset.metadata.size} • {asset.metadata.contentType}
                    </div>
                  </div>
                </CardContent>
              </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mb-4 p-3 bg-blue-50 rounded-lg">
              <h4 className="text-sm font-medium text-blue-900 mb-1">💡 Web3 Use Case:</h4>
...
// Result: ${getWalrusCDNUrl(selectedAsset.blobId, { chain: selectedChain })}`}
            ...
            <div className="space-y-3">
              ...
              {selectedAsset.availableOn.map(chain => {
                ...
                const isLoading = loadingAssets.includes(`${selectedAsset.id}-${chain}`);
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-1">
                        <span className="text-sm font-medium">
                          {chain === 'sui' ? '🌊 Sui' : chain === 'ethereum' ? '⟠ Ethereum' : '◎ Solana'}
                        </span>
                        <Badge 
                          variant="default"
            ...
            <div className="mt-4 p-4 bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg">
              <h4 className="text-sm font-medium mb-2">🎯 Why This Matters for Web3:</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                <div className="flex items-start space-x-2">
                  <span className="text-green-600">✅</span>
                  <span><strong>Developer Experience:</strong> One SDK instead of multiple integrations</span>
                </div>
                <div className="flex items-start space-x-2">
                  <span className="text-green-600">✅</span>
                  <span><strong>Performance:</strong> Faster than IPFS/Arweave gateways</span>
                </div>
                <div className="flex items-start space-x-2">
                  <span className="text-green-600">✅</span>
                  <span><strong>Reliability:</strong> Decentralized with CDN acceleration</span>
                </div>
                <div className="flex items-start space-x-2">
                  <span className="text-green-600">✅</span>
                  <span><strong>Future-proof:</strong> Easy to add new chains</span>
                </div>
              </div>
            </div>
          </CardContent>
      ...
      {selectedAsset && cidInfo && cidInfo.cid === selectedAsset.blobId && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Deep dive into cache performance and metadata for {selectedAsset.name}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-3 border rounded-lg">
                  ...
                  <div className="text-xs text-gray-500">
                    {cidInfo.cacheDate ? `Cached: ${new Date(cidInfo.cacheDate).toLocaleString()}` : 'Not cached'}
                  </div>
                </div>
              </div>
              ...
              {cidInfo.stats && (
                <div className="p-3 border rounded-lg bg-gray-50">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      <div className="font-semibold">{cidInfo.stats.requests}</div>
                      <div className="text-gray-500">Requests</div>
                    </div>
                    <div>
                      <div className="font-semibold">{cidInfo.stats.hitRate.toFixed(1)}%</div>
                      <div className="text-gray-500">Hit Rate</div>
                    </div>
                    <div>
                      <div className="font-semibold">{cidInfo.stats.avgLatency}ms</div>
                      <div className="text-gray-500">Avg Latency</div>
                    </div>
                    <div>
                      <div className="font-semibold">{(cidInfo.stats.totalSize / 1024).toFixed(1)} KB</div>
              ...
              {!cidInfo.stats && (
                <div className="p-3 border rounded-lg bg-yellow-50">
                  <p className="text-sm text-yellow-800">
                    💡 No usage statistics yet. Access this asset via the CDN to generate statistics.
                  </p>
                </div>
              )}
            </div>

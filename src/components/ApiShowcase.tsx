...
export function ApiShowcase() {
    ...
    try {
      ...
      switch (endpointId) {
        ...
        case 'preloadBlobs':
          if (!testBlobId) {
            throw new Error(
              'Please enter blob IDs (comma-separated) to preload',
            )
          }
          const blobIds = testBlobId
            .split(',')
            .map((id) => id.trim())
            .filter(Boolean)
  ...
  const selectedEndpointData = apiEndpoints.find(
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          Experience the power of WCDN's v1 API with live examples and real-time
          responses
        </p>
      </div>
          ...
          <CardContent className="space-y-3">
            {apiEndpoints.map((endpoint) => (
              <Button
                key={endpoint.id}
                variant={
                  selectedEndpoint === endpoint.id ? 'default' : 'outline'
                }

...
async function hackathonDemo() {
  ...
  Object.entries(chains).forEach(([chain, config]) => {
    ...
    colorLog(
      'white',
      `  ${statusEmoji} ${chain.toUpperCase()}: ${config.primary}`,
    )
    colorLog(
      'white',
      `     狀態: ${config.status === 'active' ? '真實 Walrus 網路' : 'Hackathon Mock'}`,
    )
  })
  ...
  colorLog('blue', '\n💻 Live Code Demo:')
  console.log(
    `${colors.magenta}import { getWalrusCDNUrl } from 'wcdn-sdk';${colors.reset}`,
  )
  ...
  console.log(
    `${colors.magenta}const suiUrl = getWalrusCDNUrl('${realBlobId}', { chain: 'sui' });${colors.reset}`,
  )
  ...
  console.log(
    `${colors.magenta}const ethUrl = getWalrusCDNUrl('${realBlobId}', { chain: 'ethereum' });${colors.reset}`,
  )
  ...
  console.log(
    `${colors.magenta}const solUrl = getWalrusCDNUrl('${realBlobId}', { chain: 'solana' });${colors.reset}`,
  )
  ...
  try {
    ...
    colorLog(
      'cyan',
      `🌐 可用的鏈: ${status.summary.availableChains.join(', ')}`,
    )

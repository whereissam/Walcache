...
async function multiChainComparisonExample() {
  ...
  try {
    ...
    const multiChainResult = await client.verifyMultiChain(chains, verificationOptions)
    ...
    chains.forEach(chain => {
      ...
      console.log(`- ${chain.toUpperCase()}:`, chainResult.hasAccess ? '✅ Verified' : '❌ Not verified')
    ...
    if (multiChainResult.recommendedEndpoint) {
      console.log('\nRecommended Endpoint:', multiChainResult.recommendedEndpoint)
    }
...
async function advancedCDNExample() {
  ...
  try {
    ...
    if (advancedResult.verification) {
      console.log('- Verification Status:', advancedResult.verification.hasAccess ? '✅ Verified' : '❌ Failed')
    ...
    if (advancedResult.nodeSelection) {
      ...
      console.log('- Selection Strategy:', advancedResult.nodeSelection.strategy)
...
async function fullWorkflowExample() {
  ...
  console.log('- Ethereum verification:', ethVerificationResult.hasAccess ? '✅ Verified' : '❌ Failed')

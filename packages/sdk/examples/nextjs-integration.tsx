...
export function useWCDNUpload() {
  ...
  const uploadWithBlockchainRegistration = useCallback(
    async (
      files: File[],
      options: {
      ...
      try {
        ...
        if (options.registerOnChain && options.chain) {
          ...
          if (blobsToRegister.length > 1) {
            ...
            console.log(
              `Registered on ${options.chain}:`,
              txHashes[options.chain],
            )
          }
        }
          ...
          <div className="space-y-2">
            {uploads.map((upload, index) => (
              <div key={index} className="border rounded p-4">
                ...
                <p>
                  <strong>Blob ID:</strong> {upload.blob_id}
                </p>
                <p>
                  <strong>CDN URL:</strong>
                  <a
                    href={wcdnClient.getCDNUrl(upload.blob_id)}
                    target="_blank"
                ...
                <p>
                  <strong>Size:</strong> {(upload.size / 1024).toFixed(2)} KB
                </p>
                <p>
        ...
        <div className="space-y-6">
          ...
          <div className="border rounded p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <span
                  className={`ml-2 px-2 py-1 rounded text-sm ${status.cache.cached ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                >
              ...
              <div>
                ...
                <span
                  className={`ml-2 px-2 py-1 rounded text-sm ${status.cache.pinned ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}
                >
              ...
              <div>
                ...
                {(status.cache.size / 1024).toFixed(2)} KB
              </div>
              <div>
                <span className="font-medium">Content Type:</span>{' '}
                {status.cache.content_type}
              </div>
            </div>
          </div>
          ...
          <div className="border rounded p-4">
            ...
            <div className="space-y-3">
              {Object.entries(status.blockchain).map(
          ...
          <div className="border rounded p-4">
            ...
            <div className="space-y-3">
              {Object.entries(status.aggregators.chains).map(
                ([chain, data]: [string, any]) => (
                  <div
                    key={chain}
                    className="flex items-center justify-between p-3 bg-gray-50 rounded"
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-1 rounded text-sm ${data.exists ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                      >
...
export async function verifyBlobIntegrity(
  ...
  for (const chain of supportedChains) {
    try {
      const isValid = await wcdnClient.verifyBlobHashOnChain(
        blobId,
        expectedHash,
        chain,
      )

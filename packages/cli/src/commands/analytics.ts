...
export const analyticsCommand = new Command('analytics').description(
  'View WCDN analytics and performance metrics',
)
    ...
    try {
      ...
      try {
        ...
        const globalData = [
          ...
          [
            'Hit Rate',
            `${((analytics.global.cache_hits / analytics.global.total_requests) * 100).toFixed(2)}%`,
          ],
          [
            'Total Bytes Served',
            formatBytes(analytics.global.total_bytes_served),
          ],
        ...
        const cacheData = [
          ...
          [
            'Redis Connected',
            analytics.cache.redis_connected ? '✓ Yes' : '✗ No',
          ],
        ]
        ...
        if (
          ...
          Object.keys(analytics.geographic).length > 0
        ) {
          console.log(chalk.bold('\n🌍 Geographic Distribution:'))
          const geoData = [
        ...
        if (analytics.application) {
          ...
          const appData = [
            ...
            [
              'Active Connections',
              analytics.application.active_connections.toLocaleString(),
            ],
            [
              'Requests/Second',
              analytics.application.requests_per_second.toFixed(2),
            ],
            [
              'Error Rate',
              `${(analytics.application.error_rate * 100).toFixed(2)}%`,
            ],
          ]
          ...
          if (hitRate > 0.9) {
            console.log(
              chalk.green('✓ Excellent cache performance (>90% hit rate)'),
            )
          } else if (hitRate > 0.7) {
            console.log(
              chalk.yellow('⚠ Good cache performance (70-90% hit rate)'),
            )
          } else {
    ...
    try {
      ...
      try {
        ...
        const basicData = [
          ...
          [
            'Hit Rate',
            `${((analytics.cache_hits / analytics.total_requests) * 100).toFixed(2)}%`,
          ],
          ['Total Bytes Served', formatBytes(analytics.total_bytes_served)],
          [
            'Last Accessed',
            new Date(analytics.last_accessed * 1000).toLocaleString(),
          ],
        ...
        // Geographic distribution
        if (
          analytics.geographic_stats &&
          Object.keys(analytics.geographic_stats).length > 0
        ) {
          console.log(chalk.bold('\n🌍 Geographic Distribution:'))
          const geoData = [
            ['Region', 'Requests', 'Share'],
        ...
        const avgBytesPerRequest =
          analytics.total_bytes_served / analytics.total_requests

        console.log(
          `• Average bytes per request: ${formatBytes(avgBytesPerRequest)}`,
        )

        if (hitRate > 0.9) {
          console.log(
            chalk.green('• Excellent cache performance for this blob'),
          )
        } else if (hitRate > 0.7) {
        ...
        const daysSinceAccess =
          (Date.now() - analytics.last_accessed * 1000) / (1000 * 60 * 60 * 24)
        if (daysSinceAccess < 1) {
          console.log(chalk.green('• Recently accessed (< 1 day ago)'))
        } else if (daysSinceAccess < 7) {
          console.log(
            chalk.yellow(
              `• Last accessed ${Math.floor(daysSinceAccess)} days ago`,
            ),
          )
        } else {
          console.log(
            chalk.red(
              `• Stale content (${Math.floor(daysSinceAccess)} days since last access)`,
            ),
          )
        }
      } catch (error) {
      ...
      try {
        ...
        if (options.format === 'json') {
          ...
          const totalRequests = result.data.reduce(
            (sum, entry) => sum + entry.total_requests,
            0,
          )
          const totalHits = result.data.reduce(
            (sum, entry) => sum + entry.cache_hits,
            0,
          )
          const totalBytes = result.data.reduce(
            (sum, entry) => sum + entry.total_bytes_served,
            0,
          )
          ...
          console.log(
            `Overall hit rate: ${((totalHits / totalRequests) * 100).toFixed(2)}%`,
          )

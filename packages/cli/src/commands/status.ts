...
export const statusCommand = new Command('status')
  ...
  .option(
    '-c, --chain <chain>',
    'specific chain to check (ethereum, sui, solana)',
  )
...
async function showBlobStatus(client: any, blobId: string, options: any) {
  ...
  try {
    ...
    if (
      ...
      client.isBlockchainIntegrationAvailable()
    ) {
      const chain = options.chain
      checks.push(
        client
          .getBlobMetadataFromChain(blobId, chain)
    ...
    if (!options.cacheOnly && !options.blockchainOnly) {
      const chains = options.allChains
        ? undefined
        : options.chain
          ? [options.chain]
          : undefined
      checks.push(
        client
          .getMultiChainBlobStatus(blobId, chains)
          .catch((error) => ({ error: error.message })),
      )
    }
      ...
      const cacheData = [
        ...
        [
          ...
          cacheStatus.cache_date
            ? new Date(cacheStatus.cache_date * 1000).toISOString()
            : 'N/A',
        ],
    ...
    if (blockchainStatus && !blockchainStatus.error) {
      ...
      const blockchainData = [
        ['Chain', 'Status', 'Uploader', 'Pinned', 'Timestamp'],
      ]
    ...
    if (aggregatorStatus && !aggregatorStatus.error) {
      ...
      const aggregatorData = [
        ['Chain', 'Status', 'Endpoint', 'Latency', 'Last Checked'],
      ]
      ...
      if (aggregatorStatus.summary.bestChain) {
        ...
        console.log(
          `Available on ${aggregatorStatus.summary.availableChains.length}/${aggregatorStatus.summary.totalChains} chains`,
        )
      }
...
async function watchBlobStatus(client: any, blobId: string, options: any) {
  ...
  console.log(
    chalk.gray(
      `Update interval: ${options.interval}s (Press Ctrl+C to stop)\n`,
    ),
  )

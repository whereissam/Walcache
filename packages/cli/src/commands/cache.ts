...
import { formatBytes, getWCDNClient, log, verbose } from '../utils/config.js'

export const cacheCommand = new Command('cache').description(
  'Manage WCDN cache operations',
)
  ...
  .action(async (blobIds, options) => {
    try {
        ...
        const { blobIdInput } = await inquirer.prompt([
          {
            ...
            validate: (input) =>
              input.trim().length > 0 || 'Please enter at least one blob ID',
          },
        ])
      ...
      try {
        ...
        spinner.succeed(
          `Preload completed: ${result.cached} cached, ${result.errors} errors`,
        )
...
cacheCommand
  ...
  .argument(
    '[blob-ids...]',
    'specific blob IDs to clear (clears all if none provided)',
  )
  .option('-y, --yes', 'skip confirmation prompts')
  ...
  .action(async (blobIds, options) => {
    try {
      ...
      if (clearAll) {
        console.log(
          chalk.bold.red('⚠️  WARNING: This will clear ALL cache entries!'),
        )
      } else {
        console.log(
          chalk.bold(`Clearing ${blobIds.length} specific cache entries:`),
        )
      ...
      // Perform clear
      const spinner = ora(
        clearAll ? 'Clearing all cache...' : 'Clearing cache entries...',
      ).start()
      ...
      try {
        ...
        if (clearAll) {
          ...
          spinner.succeed(
            `Cache clear completed: ${result.cleared || 0} cleared`,
          )
      ...
      try {
        ...
        const pinnedPercentage =
          stats.total_entries > 0
            ? ((stats.pinned_entries / stats.total_entries) * 100).toFixed(2)
            : 0
        const avgEntrySize =
          stats.total_entries > 0
        ...
        if (stats.hit_rate > 0.8) {
          ...
          console.log(
            chalk.yellow('⚠ Cache performance is good but could be improved'),
          )
        } else {

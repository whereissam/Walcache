  ...
  .action(async (files, options) => {
    try {
      ...
      if (options.chain && options.register !== false) {
        console.log(
          `Blockchain: ${options.chain || config.defaults?.chain || 'ethereum'}`,
        )
      }
      ...
      try {
        ...
        if (
          (options.chain || config.defaults?.registerOnChain) &&
          ...
          if (!client.isBlockchainIntegrationAvailable()) {
            log(
              'Blockchain integration not configured, skipping registration',
              'warning',
            )
          } else if (!client.getSupportedChains().includes(chain)) {
            log(
              `Chain "${chain}" not configured, skipping registration`,
              'warning',
            )
          } else {
            const blockchainSpinner = ora(
              `Registering on ${chain} blockchain...`,
            ).start()
            ...
            try {
              ...
              blockchainSpinner.succeed(
                `Registered ${blobsToRegister.length} blobs on ${chain} (${txHashes.length} transactions)`,
              )
        ...
        uploads.forEach((upload, index) => {
          ...
          console.log(
            `   Status: ${upload.status === 'completed' ? chalk.green('✓ Completed') : chalk.yellow('⏳ Processing')}`,
          )

          if (upload.blockchainTxs?.length > 0) {
            console.log(
              `   Blockchain: ${chalk.green('✓ Registered')} (${upload.blockchainTxs.length} tx)`,
            )
          }
        })
    ...
    const { fileInput } = await inquirer.prompt([
      {
        ...
        validate: (input) =>
          input.trim().length > 0 || 'Please enter at least one file path',
      },
    ])
  ...
  for (const filePath of files) {
    ...
    try {
      ...
      if (stat.isFile()) {
        ...
        log(
          `Skipping directory ${filePath} (use --recursive to include)`,
          'warning',
        )
      }
    } catch (error) {
...
async function walkDirectory(dirPath: string): Promise<Array<FileInfo>> {

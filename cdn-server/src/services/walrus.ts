...
class WalrusService {
      ...
      try {
        ...
        if (axios.isAxiosError(error)) {
          if (error.response?.status === 404) {
            console.warn(`Blob ${cid} not found on ${aggregatorUrl} - trying next aggregator`);
  ...
  async fetchBlobWithRetry(cid: string, maxRetries: number = 3, interval: number = 2000): Promise<WalrusBlob | null> {
    console.log(`Fetching blob ${cid} with retry (max ${maxRetries} attempts)...`);
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (error instanceof WalrusError && error.code === WALRUS_ERROR_CODES.BLOB_NOT_AVAILABLE_YET) {
          console.log(`Attempt ${attempt}/${maxRetries}: Blob ${cid} not yet synced to aggregator`);
  ...
  async uploadBlob(data: Buffer, contentType: string, epochs?: number): Promise<string> {
    try {
  ...
  async waitForAggregatorSync(blobId: string, maxRetries: number = 10, delayMs: number = 2000): Promise<boolean> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (response.status === 200) {
          console.log(`Blob ${blobId} synced to aggregator after ${attempt} attempts`);
          return true;
        }
      } catch (error) {
        if (axios.isAxiosError(error) && error.response?.status === 404) {
          console.log(`Attempt ${attempt}/${maxRetries}: Blob ${blobId} not yet synced`);
    ...
    console.warn(`Blob ${blobId} not synced to aggregator after ${maxRetries} attempts`);
    return false;
  }

  async uploadAndWaitForSync(data: Buffer, contentType: string, epochs?: number): Promise<{ blobId: string; synced: boolean }> {

  ...
  fastify.post(
    ...
    async (request: EncryptedFileRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        const contentId =
          fields.contentId?.value || sealService.generateContentId()
        const threshold = fields.threshold?.value
          ? parseInt(fields.threshold.value)
          : config.integrations.seal.defaultThreshold
        ...
        if (!sealService.validatePackageId(packageId)) {
          return reply.code(400).send({
            success: false,
            error:
              'Invalid packageId format (must be 0x followed by 64 hex characters)',
          })
        }
        ...
        console.log(
          `🔐 Encrypting file: ${filename} (${fileBuffer.length} bytes)`,
        )
  ...
  fastify.post(
    ...
    async (
      ...
      try {
        ...
        // Validate txBytes is a valid hex string
        if (typeof txBytes !== 'string' || !/^[0-9a-fA-F]+$/.test(txBytes)) {
          return reply.code(400).send({
            success: false,
            error: 'txBytes must be a valid hex string',
          })
        }

        // Validate sessionKey is an object (basic structural check)
        if (typeof sessionKey !== 'object' || sessionKey === null) {
          return reply.code(400).send({
            success: false,
            error: 'sessionKey must be a valid object',
          })
        }
